...
const SampleTracker = () => {
  ...
  const calculateQCResult = (species, contigs, length, estCov, meanQ_R1, meanQ_R2) => {
    ...
    // Default for other species (manual decision for now)
  ...
  const ProcessStepForm = () => {
    ...
    // Auto-calculate QC result when MiSeq QC fields change
    React.useEffect(() => {
          ...
          <div className="space-y-4">
            ...
            {processStep === 'MiSeq QC' && (
              <div className="space-y-3 border-t pt-3 mt-4">
                ...
                {/* Auto QC Result Display for E.coli */}
                {processStep === 'MiSeq QC' && qcResult && qcResult.details && (
                  <div className="mt-4 p-3 bg-gray-50 rounded border">
                    <h5 className="font-medium text-sm mb-2">E.coli QC Analysis:</h5>
            ...
            {processStep === 'MiSeq QC' && (
              <div>
                <label className="block text-sm font-medium mb-1">QC Result</label>
                {processingSample?.species === 'E.coli' && qcResult && qcResult.pass !== null ? (
                  <div className={`p-2 rounded ${qcResult.pass ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                    ...
                    <p className="text-sm mt-1">
                      {qcResult.pass ? 'All E.coli QC thresholds met - Ready for NCBI submission' : 'QC thresholds not met - Needs resequencing'}
                    </p>
                  </div>
                ) : (
                  <div className="flex gap-4">
        ...
        <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-96 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {sample.miseqQcData && (
              <div>
                <label className="block text-sm font-medium text-gray-500">QC Status</label>
                <p className={sample.miseqQcData.qcPass ? 'text-green-600' : 'text-red-600'}>
                  {sample.miseqQcData.qcPass ? 'PASS' : 'FAIL'}
                </p>
              </div>
            )}
            <div>
            ...
            <div>
              ...
              <p>{sample.dateWgs || sample.dateIndex || sample.dateCleanup || sample.dateDilution || sample.dateExtraction || sample.dateTsb || sample.dateBap || sample.dateReceived}</p>
            </div>
            <div className="col-span-2">
